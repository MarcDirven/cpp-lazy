...
#include "detail/iterators/take_every.hpp"
...
public:
    ...
    take_every_iterable(Iterator begin, S end, const diff_type<Iterator> offset) :
        take_every_iterable(std::move(begin), std::move(end), offset, iter_cat<Iterator>{}) {
    }
 ...
 * one can use: `take_every({1, 2, 3}, 2, 2)` the second `2` is the start indexOf, making it select only `3`.
...
LZ_NODISCARD LZ_CONSTEXPR_CXX_20 take_every_iterable<iter<Iterable>, sentinel<Iterable>>
take_every(Iterable&& iterable, const diff_type<iter<Iterable>> offset, iter<Iterable> start) {
...
template<LZ_CONCEPT_ITERABLE Iterable>
LZ_NODISCARD LZ_CONSTEXPR_CXX_20 take_every_iterable<iter<Iterable>, sentinel<Iterable>>
take_every(Iterable&& iterable, const diff_type<iter<Iterable>> offset) {
