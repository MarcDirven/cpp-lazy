...
public:
	...
	LZ_NODISCARD LZ_CONSTEXPR_CXX_14 IterView<internal::ConcatenateIterator<Iterator, internal::IterTypeFromIterable<Iterables>...>>
	...
	LZ_NODISCARD LZ_CONSTEXPR_CXX_17 IterView<internal::ExcludeIterator<Iterator>> exclude(const difference_type from,
	...
	//! See Take.hpp for documentation.
	LZ_NODISCARD constexpr IterView<internal::TakeEveryIterator<Iterator>> takeEvery(const difference_type offset,
	...
	LZ_NODISCARD LZ_CONSTEXPR_CXX_14 IterView<internal::CartesianProductIterator<Iterator, internal::IterTypeFromIterable<Iterables>...>>
	...
	template<class UnaryPredicate, class Execution = std::execution::sequenced_policy>
	LZ_NODISCARD LZ_CONSTEXPR_CXX_20
	IterView<Iterator> dropWhile(UnaryPredicate predicate, Execution execution = std::execution::seq) const {
	...
	LZ_NODISCARD LZ_CONSTEXPR_CXX_20
	auto trim(UnaryPredicateFirst first, UnaryPredicateLast last, Execution execution = std::execution::seq) const ->
	...
	LZ_NODISCARD LZ_CONSTEXPR_CXX_20
	value_type findFirstOrDefault(const T& toFind, const U& defaultValue, Execution execution = std::execution::seq) const {
	...
	LZ_NODISCARD LZ_CONSTEXPR_CXX_20
	value_type findFirstOrDefaultIf(UnaryPredicate predicate, const U& defaultValue, Execution execution = std::execution::seq) const {
	...
	template<class T, class U, class Execution = std::execution::sequenced_policy>
	LZ_NODISCARD LZ_CONSTEXPR_CXX_20
	value_type findLastOrDefault(const T& toFind, const U& defaultValue, Execution execution = std::execution::seq) const {
	...
	template<class UnaryPredicate, class U, class Execution = std::execution::sequenced_policy>
	LZ_NODISCARD LZ_CONSTEXPR_CXX_20
	value_type findLastOrDefaultIf(UnaryPredicate predicate, const U& defaultValue, Execution execution = std::execution::seq) const {
	...
	template<class UnaryPredicate, class Execution = std::execution::sequenced_policy>
	LZ_NODISCARD LZ_CONSTEXPR_CXX_20 difference_type indexOfIf(UnaryPredicate predicate, Execution execution = std::execution::seq) const {
	 ...
	 */
	template<class UnaryPredicate, class Execution = std::execution::sequenced_policy>
	LZ_NODISCARD LZ_CONSTEXPR_CXX_20 difference_type countIf(UnaryPredicate predicate, Execution execution = std::execution::seq) const {
		if constexpr (internal::checkForwardAndPolicies<Execution, Iterator>()) {
			static_cast<void>(execution);
	 ...
	 */
	template<class BinaryPredicate = std::less<>, class Execution = std::execution::sequenced_policy>
	LZ_NODISCARD LZ_CONSTEXPR_CXX_20 bool isSorted(BinaryPredicate predicate = {}, Execution execution = std::execution::seq) const {
		if constexpr (internal::checkForwardAndPolicies<Execution, Iterator>()) {
	...
	T foldr(T&& init, BinaryFunction function) const {
		auto reverseView = lz::reverse(*this);
		return internal::accumulate(
			internal::begin(std::move(reverseView)), internal::end(std::move(reverseView)), std::forward<T>(init), std::move(function));
	}
