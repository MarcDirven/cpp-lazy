...
template<LZ_CONCEPT_ITERATOR Iterator, class S, class Function>
    class Map final : public detail::basic_iterable < detail::map_iterator<Iterator, S, Function>,
...
private:
    LZ_CONSTEXPR_CXX_20 Map(Iterator begin, S end, Function function, std::forward_iterator_tag) :
        detail::basic_iterable<iterator, S>(iterator(std::move(begin), std::move(function)), std::move(end)) {
    }

    LZ_CONSTEXPR_CXX_20 Map(Iterator begin, S end, Function function, std::bidirectional_iterator_tag) :
        detail::basic_iterable<iterator>(iterator(std::move(begin), function), iterator(std::move(end), function)) {
    }

public:
    LZ_CONSTEXPR_CXX_20 Map(Iterator begin, S end, Function function) :
        Map(std::move(begin), std::move(end), std::move(function), IterCat<Iterator>{}) {
    }
...
LZ_NODISCARD LZ_CONSTEXPR_CXX_20 Map<iter<Iterable>, sentinel<Iterable>, detail::decay<Function>>
