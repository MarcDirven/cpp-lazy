...
/**
 * Performs an SQL-like join where the result of the function `a` is compared with `b, and returns `resultSelector` if those are equal.
 * The selector for a must be a function with a parameter of type = `decltype(iterableA[n])`.
 * The selector for b must be a function with a parameter of type = `decltype(iterableB[n])`.
