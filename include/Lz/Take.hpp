...
template<class Iterator>
    class TakeN : public detail::basic_iterable < detail::take_n_iterator<Iterator>,
...
private:
    LZ_CONSTEXPR_CXX_20 TakeN(Iterator begin, typename iterator::difference_type n, std::forward_iterator_tag /*unused*/) :
...
public:
    LZ_CONSTEXPR_CXX_20 TakeN(Iterator begin, typename iterator::difference_type n) :
        TakeN(std::move(begin), n, iter_cat<Iterator>{}) {
    }
...
LZ_NODISCARD LZ_CONSTEXPR_CXX_14 TakeN<iter<Iterable>> take(Iterable&& iterable, const diff_iterable_t<Iterable> n) {
    return TakeN<iter<Iterable>>(detail::begin(std::forward<Iterable>(iterable)), n);
}
