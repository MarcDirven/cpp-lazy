...
TEST_CASE("Take changing and creating elements", "[Take][Basic functionality]") {
    ...
    SECTION("Should take while range") {
        auto taken = lz::takeWhile(array, [](int i) {
            return i != 5;
        });
...
TEST_CASE("Take to containers", "[Take][To container]") {
    ...
    SECTION("To map") {
        constexpr std::size_t newSize = 2;
        std::map<int, int> actual = lz::take(array, newSize).toMap([](const int i) {
            return i;
        });
    ...
    SECTION("To map") {
        constexpr std::size_t newSize = 2;
        std::unordered_map<int, int> actual = lz::take(array, newSize).toUnorderedMap([](const int i) {
            return i;
        });
...
TEST_CASE("DropWhile changing and creating elements", "[DropWhile][Basic functionality]") {
    ...
    const auto dropped = lz::dropWhile(arr, [](const int i) {
        return i == 1;
    });
...
TEST_CASE("DropWhile binary operations", "[DropWhile][Binary ops]") {
    ...
    auto dropped = lz::dropWhile(vec, [](const int i) {
        return i == 1;
    });
    auto begin = dropped.begin();
...
TEST_CASE("DropWhile to containers", "[DropWhile][To container]") {
    ...
    auto dropped = lz::dropWhile(arr, [](const int i) {
        return i == 1;
    });
    ...
    SECTION("To map") {
        std::map<int, int> map = dropped.toMap([](const int i) {
            return i;
        });
    ...
    SECTION("To unordered map") {
        std::unordered_map<int, int> map = dropped.toUnorderedMap([](const int i) {
            return i;
        });
