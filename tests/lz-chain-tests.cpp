...
TEST_CASE("Iterator chaining") {
    ...
    auto zippedWith = lz::toIter(arr).zipWith(
        [](int a, int b) {
            return a + b;
        },
        arr2);
    ...
    CHECK(lz::toIter(arr).findFirstOrDefaultIf(
              [](int i) {
                  return i == 16;
              },
              16) == 16);
    CHECK(lz::toIter(arr).findLastOrDefault(16, 16) == 16);
    CHECK(lz::toIter(arr).findLastOrDefaultIf(
              [](int i) {
                  return i == 16;
              },
              16) == 16);
    ...
    CHECK(lz::toIter(arr).containsIf([](int i) {
        return i == 15;
    }));
    CHECK(lz::toIter(arr).foldr(0, [](int acc, int cur) {
        return acc + cur;
    }) == 120);
    ...
    CHECK(lz::toIter(arr).any([](int i) {
        return i == 0;
    }));
    CHECK(lz::toIter(arr).none([](int i) {
        return i == 16;
    }));
    CHECK(lz::toIter(arr).count(0) == 1);
    CHECK(lz::toIter(arr).countIf([](int i) {
        return i == 0;
    }) == 1);
